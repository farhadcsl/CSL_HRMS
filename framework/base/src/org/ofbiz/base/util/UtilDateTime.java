...
public class UtilDateTime {
    ...
    public static int getHourInterval(Timestamp from, Timestamp thru) {
        return thru != null ? (int) ((thru.getTime() - from.getTime()) / (60*60*1000)) : 0;
    }
    public static HashMap<String,Integer> covertLongToHourMinuteSecond(long timeInMillisecond){
    	HashMap<String,Integer> retunMap=new HashMap<String, Integer>();
    	long hour=timeInMillisecond/(3600*1000);
    	long min=(timeInMillisecond%(3600*1000))/(60*1000);
    	long sec=((timeInMillisecond%(3600*1000))%(60*1000))/1000;
    	retunMap.put("hour", (int)hour);
    	retunMap.put("minute", (int)min);
    	retunMap.put("second", (int)sec);
    	return retunMap;
    }
