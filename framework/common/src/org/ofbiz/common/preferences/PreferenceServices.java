...
public class PreferenceServices {
   ...
   /* public static Map<String, Object> setUserPreference(DispatchContext ctx, Map<String, ?> context) {
    ...
    public static Map<String, Object> setUserPreference(DispatchContext ctx, Map<String, ?> context) {
        ...
        String userLoginId = PreferenceWorker.getUserLoginId(context, false);
        String userPrefTypeId = (String) context.get("userPrefTypeId");
        String userPrefTypeIdUnit=null;
        if(userPrefTypeId.equalsIgnoreCase("ORGANIZATION_PARTY")){
        	userPrefTypeIdUnit="ORGANIZATION_UNIT";
        }
        Object userPrefValue = context.get("userPrefValue");
        if (UtilValidate.isEmpty(userLoginId) || UtilValidate.isEmpty(userPrefTypeId) || userPrefValue == null) {
            return ServiceUtil.returnError(UtilProperties.getMessage(resource, "setPreference.invalidArgument", locale));
        ...
        try {
            ...
            List<GenericValue> recList = delegator.findByAnd("UserPreference",  UtilMisc.toMap("userLoginId",userLoginId,"userPrefTypeId", userPrefTypeId,"userPrefGroupTypeId", userPrefGroupTypeId,"userPrefValue", userPrefValue));
            GenericValue rec = EntityUtil.getFirst(recList);
            if(UtilValidate.isEmpty(rec)){
            	rec = delegator.makeValidValue("UserPreference", PreferenceWorker.toFieldMap(userLoginId, userPrefTypeId, userPrefGroupTypeId, userPrefValue));
                delegator.createOrStore(rec);
            }else{
            	delegator.store(rec);
            }
            if(userPrefTypeIdUnit!=null){
            	   userPrefValue = ObjectType.simpleTypeConvert("0", userPrefDataType, null, null, false);
            	
            	   userPrefTypeId=userPrefTypeIdUnit;
            	 
            	   List<GenericValue> recunitList = delegator.findByAnd("UserPreference",  UtilMisc.toMap("userLoginId",userLoginId,"userPrefTypeId", userPrefTypeId,"userPrefGroupTypeId", userPrefGroupTypeId,"userPrefValue", userPrefValue));
                   GenericValue recunit = EntityUtil.getFirst(recunitList);
                   if(UtilValidate.isEmpty(recunit)){
                	   recunit = delegator.makeValidValue("UserPreference", PreferenceWorker.toFieldMap(userLoginId, userPrefTypeId, userPrefGroupTypeId, userPrefValue));
                       delegator.createOrStore(recunit);
                   }else{
                   	delegator.store(recunit);
                   }
