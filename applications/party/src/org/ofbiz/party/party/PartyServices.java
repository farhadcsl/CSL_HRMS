...
public class PartyServices {
    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
       ...
       /* for (GenericValue value : partyList) {
            if (isStructureAvailable(delegator, value.get("partyId").toString())) {
            	partyList.remove(value);
            }
        }*/
    ...
    public static boolean isStructureAvailable(Delegator delegator, String partyId) {
        List<GenericValue> listeligible = null;
        try {
        	listeligible = delegator.findByAnd("Employment", UtilMisc.toMap("partyIdTo", partyId,"employmentType", null));
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
        return (listeligible.isEmpty()) ? false : true;
    }
