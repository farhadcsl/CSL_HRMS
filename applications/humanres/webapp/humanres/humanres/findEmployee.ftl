...
<div id="findEmployee" class="screenlet">
    <div class="screenlet-title-bar">
        <ul>
            ...
            <#if partyList?exists><li><a href="<@ofbizUrl>findEmployees?hideFields=Y${paramList}</@ofbizUrl>">${uiLabelMap.CommonHideFields}</a></li></#if>
                <li><a href="javascript:document.lookupparty.submit();">View Employee List</a></li>
            </#if>
        </ul>
        <br class="clear"/>
    </div>
       ...
       -->
        <form method="post" name="lookupparty" action="<@ofbizUrl>findEmployees</@ofbizUrl>" class="basic-form">
            ...
            <#if extInfo == "O">
                ...
                <tr align="center">
                    <td>&nbsp;</td>
                    <td><input type="submit" value="Find Employee" onclick="javascript:document.lookupparty.submit();"/>
    ...
    <#if partyList?has_content>
        <table class="basic-table" cellspacing="0">
          ...
          <!--  <#assign partyType = partyRow.getRelatedOne("PartyType")?if_exists>
               ...
               <td>
               <#assign partyconsName = Static["org.ofbiz.party.party.PartyHelper"].getPartyConsName(partyRow, true)>
               <#if partyconsName?has_content?exists>${partyconsName}<#else>???</#if>
               </td>
                
               <!-- <td>
                
                	<#if partyRow.containsKey("statusId")>
                		<#if partyRow.statusId?has_content>
                		<#if partyRow.statusId="EMPL_POS_ACTIVE">
                           	Active
                        <#elseif partyRow.statusId="EMPL_POS_INACTIVE"> 
                    		Inactive 
                     	</#if>
                     	<#else> 
                    		NULL
    ...
    <#else>
        <div class="screenlet-body">
            <span class="h3">No Employee Found</span>
        </div>
    </#if>
