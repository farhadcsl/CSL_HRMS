...
public class HumanResEvents {
    ...
    public static void getChildHRCategoryTree(HttpServletRequest request, HttpServletResponse response){
        ...
        String businessUnitCheck="Y";
        List categoryList = FastList.newInstance();
        List<GenericValue> childOfComs;
        if(unitType==null ||   unitType.equals(businessUnitCheck) ){
                ...
                if (UtilValidate.isNotEmpty(emlpfillCtxs)) {
                    for (GenericValue emlpfillCtx : emlpfillCtxs ) {
                        ...
                        if (UtilValidate.isNotEmpty(memCtx)) {
                            ...
                            String middleName = (String) memCtx.get("middleName");
                            ...
                            try{
                                 ...
                                 try {
                                 	partyList = delegator.findByAnd("UserLogin",
                                             UtilMisc.toMap("partyId", memberId));
                                 } catch (GenericEntityException e) {
                                     e.printStackTrace();
                                 }
                                 if (UtilValidate.isNotEmpty(partyList)) {
                                     if (UtilValidate.isNotEmpty(partyList.get(0).get("userLoginId"))) {
                                    	 userLoginId =" ["+ partyList.get(0).get("userLoginId").toString()+"] ";
                                     }
                                 }
                            }
                            catch(Exception e){}
                            title = firstname +" "+middleName +" "+ lastname +" "+userLoginId;
                        }
    ...
    public static String convertNumberToBangla(String doubleValue){
    	String retval="";
    	if(doubleValue!=null){
