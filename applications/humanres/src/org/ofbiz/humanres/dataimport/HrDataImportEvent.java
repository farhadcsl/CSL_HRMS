...
public class HrDataImportEvent {
	public static final String module = HrDataImportEvent.class.getName();
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField()) {
						...
						if (extension.equals("xlsx")) {
							sheetData = HrDataImportServices.getDataFromExcelFile2007(uploadedFile);
							sheetData.remove(0);
						}
						if (extension.equals("xls")) {
							sheetData = HrDataImportServices.getDataFromExcelFile2003(uploadedFile);
							sheetData.remove(0);
						...
						if (extension.equals("xlsx") || extension.equals("xls")
							...
							for (List<String> datasList : sheetData) {
								if (datasList.size() > 0) {
									Map<String,Object> input = HrDataImportServices.setEmployeeDataImportReq(datasList);
									HrDataImportServices.createOrUpdateUser(delegator, input);
								}
							}
	...
	public static String ImportAttendanceDataFromExcel(HttpServletRequest request,
			HttpServletResponse response) {
		...
		if (isMultipart) {
			...
			try {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField()) {
						...
						if (extension.equals("xlsx") || extension.equals("xls")
							...
							for (List<String> datasList : sheetData) {
								if (datasList.size() > 0) {
									Map<String,Object> input = HrDataImportServices.setAttendanceDataImportReq(datasList);
									HrDataImportServices.addDataIntoDailyAttendance(delegator, input);
								}
							}
				...
				e.printStackTrace();
				request.setAttribute("_ERROR_MESSAGE_", "File Upload Error");
				return ("error");
			} catch (Exception e) {
				e.printStackTrace();
				request.setAttribute("_ERROR_MESSAGE_", "Data Import Error");
				return ("error");
			}
	...
	public static String ImportGoalsAndKpiDataFromExcel(HttpServletRequest request,
			HttpServletResponse response) {
				...
				while (iterator.hasNext()) {
					...
					if (!item.isFormField()) {
						...
						if (extension.equals("xlsx") || extension.equals("xls")
							...
							for (List<String> datasList : sheetData) {
								if (datasList.size() > 0) {
									Map<String,Object> input = HrDataImportServices.setGoalsAndKpiDataImportReq(datasList);
									HrDataImportServices.addDataIntoGoalsAndKpi(delegator, input);
								}
							}
...
//	public static void updateGoalsAndKpi(Delegator delegator,
//			Map<String, Object> mapData) {
//		
//		String partyId=mapData.get("partyId").toString();
...
//				
//		    try {
//				GenericValue GoalsAndKpiSetup = delegator
//					.makeValue("EmployeeGoalsAndKpi",
//							UtilMisc.toMap(GoalsAndKpi));
...
//			try { 
//				userLogin = delegator.findByPrimaryKey("UserLogin", UtilMisc.toMap("userLoginId", employeeId));				
